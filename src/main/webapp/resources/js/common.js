...
function onSearchBtn(){
	$("#frm").attr({"action":location.pathname}).submit();
}

// Move
function onMovePage(page){
	$("#frm").attr({"action":"/" + page + ".do"}).submit();
}

//Move Post
function onMovePageP(page){
	$("#frm").attr({"action":"/" + page + ".do", "method":"POST"}).submit();
}
...
// AjaxData
function ajaxData(data, url, alt){
	$.ajax({
			type: "POST",
			url : "/" + url + ".json",
			dataType: "json",
