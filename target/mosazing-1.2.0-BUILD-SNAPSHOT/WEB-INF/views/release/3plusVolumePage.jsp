...
<button type="button" onclick="javascript:onMovePage('volumePage1')">전체 보기</button>
...
<table class="table_border">
	...
	<tbody>
					...
					<c:choose>
						<c:when test="${item.cnt > 0}">
							<c:if test="${item.ord eq 0}">
								<td rowspan="${item.cnt + 1}" align="center"><input type="checkbox" name="codeChk" value="${item.code }"/></td>
								<td rowspan="${item.cnt + 1}" align="center"><input type="checkbox" name="codeFav" value="${item.code }" <c:if test="${item.favorite eq 1 }">checked</c:if>/></td>
								<td rowspan="${item.cnt + 1}">
							...
							<c:choose>
								<c:when test="${item.ord eq 0}">
									<td align="center" ><c:out value="${item.yy1 }" /></td>
									<td align="center" ><c:out value="${item.yy2 }" /></td>
									<td align="center" ><c:out value="${item.yy3 }" /></td>
								</c:when>
								<c:when test="${item.ord eq 4}">
									<td align="right" class='<c:if test="${item.y1 < 0}">red</c:if> <c:if test="${item.y1 > 9 and item.y1 < 11}">bestroe</c:if> <c:if test="${item.y1 >= 11}">goodroe</c:if>'><fmt:formatNumber value="${item.y1 }" pattern="#,###.##" /></td>
									<td align="right" class='<c:if test="${item.y2 < 0}">red</c:if> <c:if test="${item.y2 > 9 and item.y2 < 11}">bestroe</c:if> <c:if test="${item.y2 >= 11}">goodroe</c:if>'><fmt:formatNumber value="${item.y2 }" pattern="#,###.##" /></td>
									<td align="right" class='<c:if test="${item.y3 < 0}">red</c:if> <c:if test="${item.y3 > 9 and item.y3 < 11}">bestroe</c:if> <c:if test="${item.y3 >= 11}">goodroe</c:if>'><fmt:formatNumber value="${item.y3 }" pattern="#,###.##" /></td>
								</c:when>
								<c:when test="${item.ord eq 5 or item.ord eq 6}">
							...
							<c:choose>
								...
								<c:when test="${item.account eq 'ROE' }">
		...
		<%-- 					<td>${status.index % 3 } | ${status.count % 3 } | ${status.index / 3 } | ${status.count / 3 } | ${status.index % 3 eq 0 }</td> --%>
							<c:if test="${item.ord eq 0 }">
								<td rowspan="${item.cnt + 1}" align="center"><span data-index="low">저가</span><br/><span data-index="ll">하한가</span></td>
								<td rowspan="${item.cnt + 1}" align="center"><span data-index="high">고가</span><br/><span data-index="ul">상한가</span></td>
								<td rowspan="${item.cnt + 1}" align="center"><span data-index="real">현재가</span><br/><span data-index="sv">전일가</span><br/><span data-index="cv">전일대비</span>|<span data-index="cr">전일대비%</span><br/><span data-index="vol">거래량</span></td>
								<td rowspan="${item.cnt + 1}">
